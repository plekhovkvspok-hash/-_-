  ...
  function wireRepairEstimateWindow(win, baseData) {
    ...
    function collectRows(kind) {
      return Array.from(doc.querySelectorAll('tr[data-kind="' + kind + '"]')).map((row) => {
        ...
        const total = manualTotal || Math.round(unitPrice * qtyNumber(quantity));
    ...
    function enableEstimateEdit(silent) {
      ...
      bindEditEvents();
      if (!silent) setStatus('Режим правки: можно менять любые ячейки, добавлять/удалять строки и пересчитывать смету.');
    }
