(function () {
  ...
  function renderRisks() {
    ...
    const quorumCritical = risks.filter((row) => row.quorumPercent > 0 && row.quorumPercent < 51).length;
    ...
    const summary = `<div class="risk-summary-grid"><article><strong>${boost}</strong><span>Нужно усилиться</span></article><article><strong>${overdue}</strong><span>Просрочено</span></article><article><strong>${quorumCritical}</strong><span>Кворум ниже 51%</span></article><article><strong>${withoutOwner}</strong><span>Без ответственного</span></article></div>`;
    const rows = risks
      .slice()
      ...
      .map((row) => {
        ...
        return `<article class="risk-item priority-${riskPriority(row)} signal-${signal.level}"><header><strong><span class="signal-dot"></span>${escapeHtml(row.complex || 'Без ЖК')} · ${escapeHtml(row.object || 'Без МКД')}</strong><span>${escapeHtml(signal.label)} · ${escapeHtml(row.owner || 'без ответственного')}</span></header><p>${escapeHtml(row.risk.reasons.join('; '))}</p><p class="risk-meta">${escapeHtml(meta)}</p><p class="risk-action">${escapeHtml(action)}</p></article>`;
      }).join('');
    els.riskList.innerHTML = summary + rows;
  }
  ...
  function riskSignal(row) {
    const left = daysLeft(row);
    if (!row.owner || (left !== null && left < 0 && !isCompleted(row)) || (row.quorumPercent > 0 && row.quorumPercent < 25)) {
      return { level: 'red', label: 'Красный: срочно усилить' };
    }
    if ((left !== null && left <= 3 && !isCompleted(row)) || (row.quorumPercent >= 25 && row.quorumPercent < 40)) {
      return { level: 'orange', label: 'Оранжевый: усилить сегодня' };
    }
    if ((left !== null && left <= 7 && !isCompleted(row)) || (row.quorumPercent >= 40 && row.quorumPercent < 51) || row.resourceRisk || row.deviation > 0) {
  ...
  function riskAction(row) {
    ...
    if (row.quorumPercent >= 25 && row.quorumPercent < 51) return 'Действие: кворум не достигнут, оценить недостающую площадь и темп сбора.';
